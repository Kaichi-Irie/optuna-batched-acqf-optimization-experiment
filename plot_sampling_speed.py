# source: https://hub.optuna.org/visualization/plot_sampling_speed/
...
def _predict_n_completed_trials(
    ...
    for label, mean in n_completed_trials_stats.items():
        ...
        slope, intercept, _, _, _ = linregress(
            time_grids[use_train] ** 0.5, mean[use_train]
        )

        preds[label] = slope * time_grids**0.5 + intercept
...
def plot_sampling_speed(
    ...
    marker_dict: dict[str, str] | None = None,
) -> Axes:
    ...
    for label, n_completed_trials in n_completed_trials_stats.items():
        ...
        ax.scatter(
            ...
            marker=marker_dict.get(label, None),
            edgecolor="black",
        )
        ax.plot(
            preds[label],
    ...
    ax.set_xscale("log")
    ax.set_ylim(
        time_grids[0] - (time_grids[1] - time_grids[0]) / 2,
        (time_grids[-2] + time_grids[-1]) / 2,
    )
