...
def _verify_results(
    ...
    for x0 in X0:
        x_opt, fval, info = fmin_l_bfgs_b(
            func=func_and_grad, x0=x0, **kwargs_scipy
        )
...
def test_batched_lbfgsb(
    ...
    assert optimization_module._greenlet_imports.is_successful() is use_greenlet

    X0, bounds = X0_and_bounds(dim=dim, n_localopts=n_localopts)
    kwargs_ours.update(bounds=bounds)
    kwargs_scipy.update(bounds=bounds)
    _verify_results(
        X0, func_and_grad, kwargs_ours, kwargs_scipy, batched_lbfgsb_func=optimization_module.batched_lbfgsb
    )
